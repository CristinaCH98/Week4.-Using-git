...
def main(input_file, output_file, min_length, rbs_seq, upstream_distance):
    ...
    with open(output_file, "a") as f:  # Cambiado a "w" para sobrescribir
        for record in SeqIO.parse(input_file, "fasta"):
            ...
            # Filtrar ORFs por longitud mínima y presencia de RBS
            filtered_orfs = {orf for orf in orfs if len(orf[3]) >= min_length_codons and has_rbs(sequence, orf[0], rbs_seq, upstream_distance)}
            ...
            # Guardar las secuencias de proteínas en el archivo de salida
            for protein in protein_strings:
                f.write(f"{protein}\n")

            # Opcionalmente imprimir los ORFs en la consola
